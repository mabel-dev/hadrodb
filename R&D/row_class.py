...
def check_tuple_schema(data, schema):
    ...
    for i, (key, schema_info) in enumerate(schema.items()):
        ...
        if data[i] is None:
            if not is_nullable:
                raise ValueError(f"Tuple does not match schema: field '{key}' is not nullable but has value None")
        else:
            if not isinstance(data[i], expected_type):
                raise ValueError(f"Tuple does not match schema: field '{key}' has type {type(data[i])}, expected {expected_type}")
...
class Row(tuple):
        ...
        for i, (field_name, field_type) in enumerate(self._schema.items()):
            ...
            if value is None and field_type['nullable']:
                ...
                if field_type['type'] == 'VARCHAR':
                    ...
                    values.append(struct.pack(f'>H{len(value_bytes)}s', len(value_bytes), value_bytes))
                elif field_type['type'] in TYPE_DICT:
                    form, size = TYPE_DICT[field_type['type']]
    ...
    def create_class(cls, schema):
        row_class = type('RowClass', (Row,), {'_fields': [field for field in schema], '_schema': schema})
        return row_class
