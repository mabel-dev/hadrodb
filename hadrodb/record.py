...
#   ┌───────────┬────────────┬───────┐
#   │ timestamp │ value_size │ value │
#   └───────────┴────────────┴───────┘
# 
# We maintain b-tree of the primary key
#
...
class Row(tuple):
    ...
    def from_bytes(cls, data: bytes) -> tuple:
        fields = ormsgpack.unpackb(data)
        return cls(fields)

    def to_bytes(self) -> bytes:
        record_bytes = ormsgpack.packb(tuple(self))
        record_size = len(record_bytes)
        return record_size.to_bytes(4, "big") + record_bytes
